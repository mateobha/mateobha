#!/usr/bin/env python3
"""Generate an animated GitHub contribution-heatmap SVG (squares light up
one by one). Works standalone; designed to run in a GitHub Action daily so
the README stays live.
Usage: python generate_heatmap_svg.py [username] [output.svg]
"""
import sys, json, os, datetime, urllib.request

USER = sys.argv[1] if len(sys.argv) > 1 else os.environ.get("GITHUB_USERNAME", "")
OUT = sys.argv[2] if len(sys.argv) > 2 else "contrib-heatmap.svg"

if not USER:
    raise SystemExit("Usage: generate_heatmap_svg.py <username> [output.svg]")
